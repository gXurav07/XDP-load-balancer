...
__always_inline int updateThreadCount(int serv){
    __u32 key=serv-1;
    __u32 *value;
...
__always_inline int packetFromServers(void*data,void*data_end){
    ...
    // check-1
    if( iphdr->saddr == IP_ADDRESS(SERVER_1_IP) && updateThreadCount(1))
        return 1;
    // check-2
    if( iphdr->saddr == IP_ADDRESS(SERVER_2_IP) && updateThreadCount(2))
        return 2;
    // check-3
    if( iphdr->saddr == IP_ADDRESS(SERVER_3_IP) && updateThreadCount(3))
        return 3;
    return -1;
